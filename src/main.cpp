...
int main(int argc, char *argv[])
{
    ...
    for (const AutonomousSystemPtr_t asSystem : Network::autonomousSystems) {
        qDebug() << "AS ID:" << asSystem->getID();
        QString topologyTypeStr;
        switch (asSystem->getTopologyType()) {
        ...
        qDebug() << "Node Count:" << asSystem->getNodeCount();
        qDebug() << "AS Gateways Count:" << asSystem->getASGateways().size();
        qDebug() << "User Gateways Count:" << asSystem->getUserGateways().size();
        qDebug() << "DHCP Server Set:" << (asSystem->getDHCPServer() ? "Yes" : "No");
        qDebug() << "Broken Routers Count:" << asSystem->getBrokenRouters().size();
        qDebug() << "Connections Count:" << asSystem->getConnections().size();
    }
